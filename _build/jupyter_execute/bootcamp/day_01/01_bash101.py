...
# 1. On Windows - There are multiple ways to start Bash in Windows 10. You can use the built-in Command Prompt, PowerShell, or Bourn shell itself. Click Start, All Apps, under the letter B click Bash on Ubuntu for Windows.
# 1. Opening a terminal using jupyter cloud -  Go to [jupyter-cloud](https://jupyter-cloud.gwdg.de/), login and press on the Terminal button in the Launcher
# ```
# #### Command prompt
...
# #### Use `man` linux command to access the linux manual 
# 
# The Linux [man-pages](https://www.kernel.org/doc/man-pages/) project documents the Linux OS. 
# Every command, system call or devices are documented in this huge project. 
# And this is also accessible from the terminal itself 
# 
# 
# `````{admonition} Challenge
...
# man mkdir
# ```
# To leave the manual screen you just press the letter  <kbd>q</kbd>
# ````
# `````

# #### mkdir gives us the ability to form filesystem structures
# 
# Some of the options are quite powerful 
# 
# `````{admonition} Challenge
# Try to identify in the manual a way to create inside the folder sandbox the following structure using a single command: 
# 
# - root  
#   - dir_a  
#   - dir_b  
